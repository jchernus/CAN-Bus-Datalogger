...
def update_database():
        ...
        for PID in PIDs:
                ...
                #receive message
                p = subprocess.Popen("candump -t A -n 1 -T 350 can0,7EB:7ff", cwd="/data/can-utils/", stdout=subprocess.PIPE, shell=True)
                (output, err) = p.communicate()

                if len(output) > 0:  # got the response message

                        cellVDict[PID] = output.strip().split("  ")[4].strip()
                                ...
                                if len(output) > 0:  # got the response message
                                        ...
                                        for line in lines:
                                                try:
                                                    ...
                                                    cellVDict[PID] = cellVDict[PID] + data[4].strip()[2:]  
                                                except:
