	...
	for date in dates:
                ...
                with open(csv_path + filename, 'w+') as file:
                        ...
                        for row in curs.execute("SELECT * FROM summary WHERE date = '" + str(date).strip() + "' LIMIT 1"):
                                ...
                                file.write(str_row)
                        file.write('\nDate, Time, SOC [%], Battery Current [A], Battery Voltage [V], Battery Power Out (Operating) [kW], Battery Power In (Charging)[kW], Motor Current [AC A rms], Motor Voltage [AC V rms], Motor Controller Battery Current [A], Motor Controller Capacitor Voltage [V], Vehicle Speed [km/h], Motor Velocity [RPM], Current Highest Priority Fault, Traction State, Maximum Battery Discharge Current [A], Maximum Battery Charge Current [A], Motor Temperature [Celcius], Motor Controller Heatsink Temperature [Celcius], Battery Pack Highest Temperature [Celcius], Batt High Temp ID, Batter Pack Lowest Temperature [Celcius], Batt Low Temp ID, Highest Cell Voltage [V], Highest Cell Voltage ID, Lowest Cell Voltage [V], Lowest Cell Voltage ID, Plugged In, Charging, Operating, Running\n')
