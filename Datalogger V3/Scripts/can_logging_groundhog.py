...
battery_current1 = battery_voltage1 = mc_cap_voltage1 = heatsink_temp1 = motor_temp1 = motor_current1 = motor_voltage1 = mc_battery_current1 = motor_velocity1 = torque1 = target_torque1 = active_fault1 = 0
battery_current2 = battery_voltage2 = mc_cap_voltage2 = heatsink_temp2 = motor_temp2 = motor_current2 = motor_voltage2 = mc_battery_current2 = motor_velocity2 = torque2 = target_torque2 = active_fault2 = 0
battery_current3 = battery_voltage3 = mc_cap_voltage3 = heatsink_temp3 = motor_temp3 = motor_current3 = motor_voltage3 = mc_battery_current3 = motor_velocity3 = torque3 = target_torque3 = active_fault3 = 0
...
def parse_data(msg_id, data):
    ...
    global battery_current1, battery_voltage1, mc_cap_voltage1, heatsink_temp1, motor_temp1, motor_current1, motor_voltage1, mc_battery_current1, motor_velocity1, torque1, target_torque1, active_fault1, status_word1
    global battery_current2, battery_voltage2, mc_cap_voltage2, heatsink_temp2, motor_temp2, motor_current2, motor_voltage2, mc_battery_current2, motor_velocity2, torque2, target_torque2, active_fault2, status_word2
    global battery_current3, battery_voltage3, mc_cap_voltage3, heatsink_temp3, motor_temp3, motor_current3, motor_voltage3, mc_battery_current3, motor_velocity3, torque3, target_torque3, active_fault3, status_word3
    ...
    elif (msg_id == "479" or msg_id == "480"):     # 479 & 480 two transmit the same info
        batt_high_temp = int(data[2] + data[3] + data[0] + data[1], 16)
        batt_high_temp = twos_comp(batt_high_temp, 16)
        
        batt_low_temp = int(data[6] + data[7] + data[4] + data[5], 16)
        batt_low_temp = twos_comp(batt_low_temp, 16)
        
        batt_high_temp_id = int(data[10] + data[11] + data[8] + data[9], 16)
        batt_low_temp_id = int(data[14] + data[15] + data[12] + data[13], 16)
...
while (True): #Checks the date, starts logging, when the logging ends (end of day, or end of time-period) it will transfer data to permanent location.
    ...
    #get x messages
    p = subprocess.Popen("./candump -t A -n 12 can0,201:7ff,202:7ff,203:7ff,301:7ff,302:7ff,303:7ff,391:7ff,392:7ff,393:7ff,397:7ff,398:7ff,399:7ff", cwd="/data/can-test_pi2/", stdout=subprocess.PIPE, shell=True)
    (output, err) = p.communicate()
    lines = output.strip().split("\n")

    p = subprocess.Popen("./candump -t A -n 4 can0,477:7ff,478:7ff,479:7ff,3ff:7ff", cwd="/data/can-test_pi2/", stdout=subprocess.PIPE, shell=True)
    (output, err) = p.communicate()
    ...
    if time_span >= 1:
        ...
        command += str(motor_voltage1) + "','"
        command += str(mc_battery_current1) + "','"
        ...
        command += str(motor_voltage2) + "','"
        command += str(mc_battery_current2) + "','"
        ...
        command += str(motor_voltage3) + "','"
        command += str(mc_battery_current3) + "','"
